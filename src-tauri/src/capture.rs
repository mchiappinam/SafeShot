...
pub fn do_capture() -> Result<Vec<ScreenData>, String> {
    ...
    // Parallelize: capture all screens, then process (PNG encode + base64) in parallel
    let handles: Vec<_> = screens.into_iter().map(|screen| {
        ...
        std::thread::spawn(move || {
            ...
            // Use fast PNG compression since this is only for display, not saving
    ...
    for handle in handles {
        results.push(handle.join().map_err(|_| "Thread panicked".to_string())??);
    }
